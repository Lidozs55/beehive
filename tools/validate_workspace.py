...
def main():
    ...
    check("unique_116_units",len(ids)==116 and len(known)==116)
    check("three_volume_counts",all(sum(x.startswith(p) for x in ids)==n for p,n in [("U",38),("M",36),("D",42)]))
    check("unit_sequences",ids==[f"{p}{i:02}" for p,n in [("U",38),("M",36),("D",42)] for i in range(1,n+1)])
    ...
    check("unit_budget_1122000",sum(u["target_chars"] for u in units)==1122000==plan["target_chars"])
    check("348_publish_chapters",len(pub)==348 and [p["id"] for p in pub]==[f"P{i:03}" for i in range(1,349)])
    ...
    check("58_unique_promises",len(fid)==58==len(fset))
    check("42_main_16_seventh",fset=={f"F{i}" for i in range(1,43)}|{f"F-7-{i}" for i in range(1,17)})
